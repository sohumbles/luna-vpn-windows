...
if($State.settings.language -eq 'English'){
    $languageMap=[ordered]@{
        ...
        'Отправлять анонимные диагностические отчёты'='Send anonymous diagnostic reports'
        'Анонимные отчёты о работе'='Anonymous diagnostic reports';'Выбор можно изменить в настройках Luna.'='You can change this choice in Luna settings.'
        'Разрешить анонимные диагностические отчёты для поиска ошибок?'='Allow anonymous diagnostic reports to help find errors?'
        'При ошибках Luna отправит версию приложения и Windows, модуль и очищенное описание. VPN-конфигурации, UUID, ключи, IP-адрес, пути пользователя и история сайтов удаляются до отправки.'='When an error occurs, Luna sends the app and Windows versions, module name, and a sanitized description. VPN configurations, UUIDs, keys, IP addresses, user paths, and browsing history are removed before sending.'
